...
public class MyLocationOverlay extends Layer implements LocationListener {
    ...
    public synchronized boolean enableMyLocation(boolean centerAtFirstFix) {
        ...
        Location bestKnown = fastFix();
        if (bestKnown != null) onLocationChanged(bestKnown);
        return true;
    }
     ...
     */
    private Location fastFix() {
        Location bestLocation = null;
        if (this.locationManager != null) {
            final List<String> providers = this.locationManager.getAllProviders();
            if (providers.size() > 0) {
                for (String provider : providers) {
                    final Location lastLocation = this.locationManager.getLastKnownLocation(provider);
